...
### 2. MiroThinker：通过模型、上下文与交互式扩展突破开源研究智能体的性能边界
...
### 3. Uni-MoE-2.0-Omni：基于先进混合专家架构、训练策略与数据技术的语言核心型全模态大模型扩展
...
### 4. Souper-Model：简单算术如何实现最先进大语言模型性能
...
### 8. TiViBench：面向视频生成模型的视觉思维推理基准测试框架
...
### 9. PhysX-Anything：基于单张图像生成仿真就绪的物理三维资产
...
### 13. OlmoEarth：面向多模态地球观测的稳定潜在图像建模
...
### 18. 测试时频谱感知潜空间导向在视觉语言模型零样本泛化中的应用
...
### 22. LoCoBench-Agent：长上下文软件工程中LLM智能体的交互式基准测试框架
...
### 24. AI-Salesman：构建可信赖大语言模型驱动的电话营销系统
...
**摘要：**
大语言模型已在广泛任务中展现出卓越性能，然而多数高性能模型仍保持闭源或部分开放，限制了研究的透明度与可复现性。本研究推出Instella系列——完全开放的三十亿参数语言模型，其训练全程基于公开可获取的数据与代码库。依托AMD Instinct MI300X GPU的算力支持，该模型通过大规模预训练、通用指令微调及人类偏好对齐三个阶段完成开发。尽管使用的预训练标记数量显著少于同期多数模型，Instella在完全开放模型中取得了领先性能，并与同类规模的顶尖开放权重模型表现相当。我们同时发布两个专业变体：支持128K标记上下文长度的Instella-Long，以及通过监督微调与数学任务强化学习增强的推理专用模型Instella-Math。这些成果共同确立了Instella作为透明、高效、多功能的开源替代方案，有力推进了开放可复现语言模型研究的发展目标。
...
### 26. 一种基于区块链保障来源可靠性的去中心化检索增强生成系统
